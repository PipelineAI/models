...
def convert_to(data_set, name:str, data_directory:str, max_examples:int=None, num_shards:int=1):
    ...
    if not max_examples or max_examples > num_examples:
        max_examples = num_examples

    def _process_examples(start_idx:int, end_index:int, filename:str):
        with tf.python_io.TFRecordWriter(filename) as writer:
            for index in range(start_idx, end_index):
                sys.stdout.write(f"\rProcessing sample {index+1} of {max_examples} within a total of {num_examples}")
                sys.stdout.flush()
    ...
    if num_shards == 1:
        _process_examples(0, max_examples, _data_path(data_directory, name))
    else:
        samples_per_shard = max_examples // num_shards
...
def convert_to_tf_record(data_directory:str):
    ...
    convert_to(mnist.validation, 'validation', data_directory, max_examples=500)
    convert_to(mnist.train, 'train', data_directory, num_shards=4, max_examples=4000)
    convert_to(mnist.test, 'test', data_directory, max_examples=500)
